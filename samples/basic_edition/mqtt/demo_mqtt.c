...
#define MQTT_BASIC_THREAD_STACK_SIZE 	8096
...
int tc_iot_basic_mqtt_init(void) 
{
    int ret = 0;
...
//    setbuf(stdout, NULL);
	...
	tc_iot_mqtt_client_config *p_client_config = &g_client_config;

	if(TC_IOT_SUCCESS != tc_iot_basic_mqtt_init())
	{
		 tc_iot_hal_printf("tc_iot_basic_mqtt_init fail\n");
		 return;
	}
    ...
    ret = tc_iot_mqtt_client_subscribe(p_client, sub_topic, TC_IOT_QOS1, _on_message_received, NULL);
    if (ret != TC_IOT_SUCCESS) 
	{
...
int tc_mqtt_basic_example(void)
{
    rt_err_t result;
    rt_thread_t tid;
    ...
    {
        tc_iot_hal_printf("mqtt_basic_example has already started!");
        return 0;
    }    
    ...
    if (!tid)
    {
        tc_iot_hal_printf("no memory for thread: mqtt_basic_example");
        return -1;
    }

	tc_iot_hal_printf("TC MQTT Basic Exmaple,TC_IOT_SDK_VER:%s", TC_IOT_SDK_VERSION);
