...
#define HTTP_RPC_THREAD_STACK_SIZE  8096
...
static rt_thread_t http_rpc_tid;
...
void tc_http_rpc_thread(void) {
   ...
   // parse_command(&g_device_info, argc, argv);
	...
	rt_thread_exit();
	rt_free(http_rpc_tid);
    return;
}



int tc_http_rpc_example(void)
{
    rt_err_t result;
    
    int stack_size = HTTP_RPC_THREAD_STACK_SIZE;
    int priority = 20;
    char *stack;


    http_rpc_tid = rt_malloc(RT_ALIGN(sizeof(struct rt_thread), 8) + stack_size);
    if (!http_rpc_tid)
    {
        tc_iot_hal_printf("no memory for thread: http_rpc_example");
        return -1;
    }

	
	tc_iot_hal_printf("TC_http_rpc_example,TC_IOT_SDK_VERSION=%s", TC_IOT_SDK_VERSION);
    stack = (char *)http_rpc_tid + RT_ALIGN(sizeof(struct rt_thread), 8);
    result = rt_thread_init(http_rpc_tid,
                            "http_rpc",
                            tc_http_rpc_thread, NULL, // fun, parameter
    ...
    if (result == RT_EOK)
    {
        rt_thread_startup(http_rpc_tid);
    }
...
#include <finsh.h>
FINSH_FUNCTION_EXPORT(tc_http_rpc_example, startup http rpc example);
#endif

#ifdef FINSH_USING_MSH
MSH_CMD_EXPORT(tc_http_rpc_example, startup http rpc example);
#endif
